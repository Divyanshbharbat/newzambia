...
axios.interceptors.request.use(
  (config) => {
    ...
    if (userCollege) {
      if (!config.headers["x-college-name"]) {
...
export const downloadStudentsExcel = async (session?: string, college?: string) => {
  const currentSession = session || localStorage.getItem("selectedSession") || "";
  const currentCollege = college || localStorage.getItem("userCollege") || "";
  return await axios.get(`${root}/excelstudents`, {
    ...
    headers: {
      ...(currentSession ? { "x-session": currentSession } : {}),
      ...(currentCollege ? { "x-college-name": currentCollege } : {}),
    },
    responseType: 'blob',
  });
};

export const downloadStudentsCSV = async (session?: string, college?: string) => {
  const currentSession = session || localStorage.getItem("selectedSession") || "";
  const currentCollege = college || localStorage.getItem("userCollege") || "";
...
export const fetchHostelData = async () => {
  ...
  export const fetchStudentFees = async (standard?: string, rollNo?: string, name?: string) => {
    return await axios.get(`${root}/fees/details`, {
      params: { 
        standard: standard ? standard.trim() : undefined, 
        roll_no: rollNo ? rollNo.trim() : undefined,
...
export const getInstitutionNameAndLogo = async (year?: string, college?: string) => {
  const currentYear = year || localStorage.getItem("selectedSession") || "";
  const currentCollege = college || localStorage.getItem("userCollege") || "";
  try {
    const response = await axios.get(`${root}/getChanges`, { params: { year: currentYear, college: currentCollege } });
    const { Institution_name, SchoolLogo, SchoolAddress } = response.data;
    return { Institution_name, SchoolLogo, SchoolAddress };
  } catch (error) {
    console.error('Error fetching institution info:', error);
    return { Institution_name: 'ST.VINCENT PALLOTTI SCHOOL ZAMBIA', SchoolLogo: null, SchoolAddress: 'WESTWOOD, LUSAKA, ZAMBIA' };
  }
};
