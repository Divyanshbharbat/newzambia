...
router.get('/excelstudents', async (req, res) => {
  ...
  const college = req.headers['x-college-name'] || req.query.college || req.college;
  try {
    let whereClause = {};
    if (session) whereClause.session = session;
    if (college) whereClause.college = college;

    let count = await prisma.student.count({ where: whereClause });
    if (count === 0 && college) {
      delete whereClause.college;
    }
...
router.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const targetCollege = req.headers['x-college-name'] || req.college || 'svpcet';

    // Verify classes (Standards) exist in database for targetCollege upfront
    let dbStandards = await prisma.standards.findMany({
      where: { college: targetCollege }
    });

    if (dbStandards.length === 0) {
      dbStandards = await prisma.standards.findMany({});
    }
...
router.get("/standard/:std", async (req, res) => {
  try {
    ...
    const standard = await prisma.standards.findFirst({
      where: { std: std, college: req.college },
    });
    if (!standard) {
