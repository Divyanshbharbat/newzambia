...
# =========================================================
# STEP 14 - CONFIGURE AUTOMATED WEEKLY BACKUP & 2-MONTH RETENTION
# =========================================================

Write-Host "[14] Configuring Automated Weekly Database Backup & 60-Day Retention..." -ForegroundColor Cyan

$backupJobScript = "$baseFolder\backup_job.ps1"
...
if (-not `$scriptDir) { `$scriptDir = "$baseFolder" }

`$backupsFolder = Join-Path `$scriptDir "backups"
`$backendBackupsFolder = Join-Path `$scriptDir "newzambia\backend\backups"
...
Set-Content -Path $backupJobScript -Value $backupScriptContent -Encoding ASCII
Write-Host "[OK] Backup PowerShell script created: $backupJobScript" -ForegroundColor Green

# Register Windows Scheduled Task for Weekly Execution
try {
    $taskName = "School_ERP_Weekly_Backup"
    $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -File `"$backupJobScript`""
    ...
    Write-Host "[OK] Windows Scheduled Task registered: '$taskName' (Runs every Sunday at 02:00 AM)" -ForegroundColor Green
} catch {
    try {
        schtasks.exe /create /tn "School_ERP_Weekly_Backup" /tr "powershell.exe -ExecutionPolicy Bypass -File `"$backupJobScript`"" /sc weekly /d SUN /st 02:00 /f | Out-Null
        Write-Host "[OK] Scheduled task registered via schtasks.exe: 'School_ERP_Weekly_Backup'" -ForegroundColor Green
    } catch {
        Write-Host "[WARN] Could not register Windows Scheduled Task automatically. Node.js backend scheduler will handle weekly backups." -ForegroundColor Yellow
    }
}
Write-Host ""
...
Write-Host ""
Write-Host "Automated System Startup & Backup Details:" -ForegroundColor Cyan
Write-Host "  - Target Launcher:  $vbsLauncher"
Write-Host "  - Backup Script:    $backupJobScript"
Write-Host "  - Backup Task Name: School_ERP_Weekly_Backup (Every Sunday 2:00 AM, 60-day auto-prune)"
Write-Host ""
...
Write-Host "  - Run backup now:   Execute '$backupJobScript' in PowerShell"
